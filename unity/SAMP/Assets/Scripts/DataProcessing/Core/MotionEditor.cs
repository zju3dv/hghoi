...
public class MotionEditor : MonoBehaviour
{
    ...
    public bool ObjectCallbacks = true;
    public bool ManualSetObject = false;
    public int ManualSeed = 0;
    public bool Mirror = false;
    public bool Visualise = true;
    public bool Settings = false;

    protected bool CameraFocus = false;
    protected float FocusHeight = 1f;
    protected float FocusDistance = 1f;
    protected float FocusAngle = 90f;
    protected float FocusSmoothing = 0.05f;
    ...
    [SerializeField] protected MotionData Data = null;
    [SerializeField] protected Actor Actor = null;
    ...
    public void SetObjectCallbacks(bool value)
    {
        if (ObjectCallbacks != value)
        {
            ObjectCallbacks = value;
            LoadFrame(Timestamp);
        }
    }
    public void SetManualSetObject(bool value)
    {
        if (ManualSetObject != value && !ObjectCallbacks)
        {
            ManualSetObject = value;
            LoadFrame(Timestamp);
        }
    }
        ...
        if (Data != data)
        {
            ...
            if (Data != null)
            {
                ...
                else
                {
                    ...
                    //Data.AddModule(Module.ID.Style);
                    StyleModule stylemodule = (StyleModule)data.GetModule(Module.ID.Style);
                    stylemodule.Inspect = true;
                ...
                if (data.GetModule(Module.ID.CylinderMap) == null)
                {
                    data.AddModule(Module.ID.CylinderMap);
                }
                CylinderMapModule module = (CylinderMapModule)data.GetModule(Module.ID.CylinderMap);
                module.Size = 2f;
                module.Inspect = true;
                module.Visualise = true;
                module.Resolution = 5;
                module.Layers = 5;
                module.Overlap = true;
                module.Mask = LayerMask.GetMask("Interaction");

                if (data.GetModule(Module.ID.CuboidMap) == null)
                {
                    data.AddModule(Module.ID.CuboidMap);
                }
                CuboidMapModule module_cuboid = (CuboidMapModule)data.GetModule(Module.ID.CuboidMap);
                module_cuboid.Size = Vector3.one * 2;
                module_cuboid.Resolution = new Vector3Int(8, 8, 8);
                module_cuboid.Mask = LayerMask.GetMask("Interaction");
    ...
    public void ChangeObject(int seed = 0)
    {
        if (!ObjectCallbacks)
        {
            ManualSeed = seed;
        ...
        {
            Debug.Log("ObjectCallback is true, we cannot change object!");
        }
    }

    public void SetAllObjectInactive()
    {
        ...
        if (instance.transform.childCount > 0)
        {
            ...
            // Find that object set false and set true makes program very slow
    ...
    public void LoadFrame(float timestamp)
    {
        ...
        if (!(ObjectCallbacks || ManualSetObject))
        {
            GameObject instance = scene.GetRootGameObjects()[0];
            instance.transform.localScale = Vector3.one.GetMirror(Mirror ? Data.MirrorAxis : Axis.None);
            if (instance.transform.childCount > 0)
            {
                ...
                for (int i = 0; i < instance.transform.GetChild(0).childCount; i++)
                {
                    instance.transform.GetChild(0).GetChild(i).gameObject.SetActive(i == 0);
                }
                // Find that object set false and set true makes program very slow
                //instance.transform.GetChild(0).GetChild(0).gameObject.SetActive(true);
        ...
        if (frame.Index % L == 0 && (ObjectCallbacks || ManualSetObject))
        {
            ...
            foreach (GameObject instance in scene.GetRootGameObjects())
            {
                ...
                foreach (SceneEvent e in instance.GetComponentsInChildren<SceneEvent>(true))
                {
                    ...
                    else if (ManualSetObject)
                    {
                        e.Callback(this, ManualSeed);
                    }
                    else
                    {
                        e.Identity(this);
        ...
        //for (int k = 0; k < actor.Bones.Length; k++)
        //{
        //    Vector3 pos = actor.Bones[k].Transform.position.GetRelativePositionTo(root);
        //    Debug.Log(k + "-" + pos[0] + "-" + pos[1] + "-" + pos[2]);
        //}
    }
                ...
                using (new EditorGUILayout.VerticalScope("Box"))
                {
                    ...
                    if (Target.Data != null)
                    {
                        ...
                        using (new EditorGUILayout.VerticalScope("Box"))
                        {
                            ...
                            {
                                Target.ManualSetObject = false;
                                Target.SetObjectCallbacks(!Target.ObjectCallbacks);
                            }
                            EditorGUILayout.EndHorizontal();

                            EditorGUILayout.BeginHorizontal();
                            if (Utility.GUIButton("ManualSetObject", Target.ManualSetObject ? UltiDraw.Cyan : UltiDraw.LightGrey, UltiDraw.Black))
                            {
                                Target.SetManualSetObject(!Target.ManualSetObject);
                                Target.ChangeObject();
                            }
                            if (Utility.GUIButton("ChangeObject", UltiDraw.Grey, UltiDraw.White))
                            {
                                Target.ChangeObject(UnityEngine.Random.Range(0, 100));
                            }
                            EditorGUILayout.EndHorizontal();
