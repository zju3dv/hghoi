...
public class MotionData : ScriptableObject
{
    ...
    public void Load()
    {
        ...
        //Open Scene
        GetScene();
        EditorCoroutines.StartCoroutine(LoadCAD(), this);
    }

    public IEnumerator LoadCAD()
    {
        Scene scene = GetScene();
        string PrefabsFolder = "Assets/Palette/DataAugmentation/";
        string[] objectNames = { "armchair", "highstool", "table", "sofa", "chair_mo", "reebokstep" };
        ...
        //// Uncomment this to delete root object in the scene
        //var o = GameObject.Find("Scene");
        //DestroyImmediate(o);

        if (scene.rootCount == 0)
        {
            var emptyroot1 = new GameObject("Scene");

            foreach (var objectname in objectNames)
            {
                if (name.Contains(objectname))
                {
                    PrefabUtility.InstantiatePrefab((GameObject)AssetDatabase.LoadMainAssetAtPath(Path.Combine(PrefabsFolder, objectname + ".prefab")), emptyroot1.transform);
                    break;
                }

                else if (name.Contains("lie_down"))
                {
                    PrefabUtility.InstantiatePrefab((GameObject)AssetDatabase.LoadMainAssetAtPath(Path.Combine(PrefabsFolder, "sofa_large.prefab")), emptyroot1.transform);
                    break;
                }
            }
        }
    ...
    public void DetectSymmetry()
    {
        ...
        for (int i = 0; i < Source.Bones.Length; i++)
        {
            ...
            //M: Add this to account for smplx names
            if (Source.Bones[i].Name.Contains("left"))
            {
                int pivot = Source.Bones[i].Name.IndexOf("left");
                Hierarchy.Bone bone = Source.FindBone(Source.Bones[i].Name.Substring(0, pivot) + "right" + Source.Bones[i].Name.Substring(pivot + 4));
