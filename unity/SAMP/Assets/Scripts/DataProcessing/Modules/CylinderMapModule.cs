...
public class CylinderMapModule : Module {
	...
	public CylinderMap GetCylinderMap(Matrix4x4 sensor_transform)
	{
		CylinderMap sensor = new CylinderMap(Size, Resolution, Layers, Overlap);
		sensor.Sense(sensor_transform, Mask);
		Samples = sensor.Points.Length;
		return sensor;
	}


	public CylinderMap GetCylinderMap(Frame frame, bool mirrored, float size, int resolution, int layers)
	{
		CylinderMap sensor = new CylinderMap(size, resolution, layers, Overlap);
		RootModule module = (RootModule)Data.GetModule(ID.Root);
		if (module != null)
