...
public class RootModule : Module
{
    ...
    public void DetectSetup()
    {
        if (true || Data.SAMP)
        {
            MotionData.Hierarchy.Bone rs = Data.Source.FindBoneContains("right_shoulder");
            RightShoulder = rs == null ? 0 : rs.Index;
            MotionData.Hierarchy.Bone ls = Data.Source.FindBoneContains("left_shoulder");
            LeftShoulder = ls == null ? 0 : ls.Index;
            MotionData.Hierarchy.Bone rh = Data.Source.FindBoneContains("right_hip");
            RightHip = rh == null ? 0 : rh.Index;
            MotionData.Hierarchy.Bone lh = Data.Source.FindBoneContains("left_hip");
            LeftHip = lh == null ? 0 : lh.Index;
            MotionData.Hierarchy.Bone n = Data.Source.FindBoneContains("neck");
            Neck = n == null ? 0 : n.Index;
            MotionData.Hierarchy.Bone h = Data.Source.FindBoneContains("pelvis");
    ...
    public void PrintVec(string prefix, Vector3 temp)
    {
        Debug.Log(prefix + ": x=" + temp[0] + " y=" + temp[1] + " z=" + temp[2]);
    }
