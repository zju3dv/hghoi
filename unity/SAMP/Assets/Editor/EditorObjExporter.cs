        ...
        if (exportedObjects > 0)
        {
         ...
         for (int i = 0; i < mfList.Count; i++)
         {
          mf[i] = (MeshFilter)mfList[i];
            ...
            System.IO.Directory.CreateDirectory(targetFolder + "/" + filename + "/models");
                selection[0].position = Vector3.zero;
                selection[0].localScale = Vector3.one;
                selection[0].rotation = Quaternion.identity;
	              }
            else
            {
                filename = EditorApplication.currentScene + "_" + exportedObjects;
         ...
         MeshesToFile(mf, targetFolder + "/" + filename + "/models", "model_normalized");
